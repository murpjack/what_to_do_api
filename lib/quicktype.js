...
const seedsPath = `${__dirname}/../src/seeds/`;
readdirF(seedsPath, {})
  .pipe(
    chain((fileNames) =>
      parallel(fileNames.length)(
        fileNames.map((fileName) => readJsonF(seedsPath + fileName)),
      ),
    ),
  )
...
// TODO: Rewrite 'quicktypeJson' using Futures
